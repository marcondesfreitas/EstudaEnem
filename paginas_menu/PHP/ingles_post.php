...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    // Exibir a imagem
    $sql = mysqli_query($conn, "SELECT imagem FROM ingles");
    if (!$sql) {
      die('Erro na consulta SQL: ' . mysqli_error($conn));
    }
    $imagem = $row["imagem"];
    ...
    if($adm == 1){
      echo "
        <form action='../editar_pg/editar_ingles.php' method='post' style='display:inline;'>
          <input type='hidden' name='id' value='" . $row["id"] . "'>
          <button class='editarcont' type='submit'><img class='imagem_editar' src='../../img/lapis_conteudo.png' alt='lixeira'></button>
        </form>
        <form action='../apagar/apagar_ingles.php' method='post' style='display:inline;'>
          <input type='hidden' name='id' value='" . $row["id"] . "'>
