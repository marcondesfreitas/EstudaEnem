...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<form action='../editar_pg/editar_geografia.php' method='post' style='display:inline;'>"; // Adicionar o formulário de edição
    echo "<input type='hidden' name='id' value='" . $row["id"] . "'>"; // Enviar o ID do post como um campo oculto
    echo "<input type='submit' value='Editar'>";
    echo "</form>";
    echo "<form action='../apagar/apagar_geografia.php' method='post' style='display:inline;'>"; // Adicionar o formulário de exclusão
    echo "<input type='hidden' name='id' value='" . $row["id"] . "'>"; // Enviar o ID do post como um campo oculto
